...
1. 启动 Phase 6：RAG 回归数据集（50+ 用例 + JUnit 自动化）。
2. 浏览器联调验证 Chunk 行内角标溯源和 Query Rewrite 实际效果。
3. 扩充知识库文档数量，优先补充稳定可引用的校园制度、通知、表格和 FAQ。
4. 提交前运行 `clean testClasses` 和全量 `test`，保持 56/56 后端测试通过基线。
